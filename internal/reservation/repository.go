...
func (r *Repository) CheckAvailability(ctx context.Context, resourceID uuid.UUID, start, end time.Time) (bool, error) {
	var count int
	// Query ini memastikan tidak ada irisan waktu sedikitpun
	query := `
		SELECT COUNT(*) 
		FROM bookings 
		WHERE resource_id = $1 
...
func (r *Repository) CreateWithItems(ctx context.Context, b Booking, itemIDs []uuid.UUID) error {
	...
	queryBooking := `
		...
		VALUES (:id, :tenant_id, :customer_id, :resource_id, :start_time, :end_time, :access_token, :status)`
	...
	// 3. Insert Booking Options (Jika ada items)
	if len(itemIDs) > 0 {
		queryItem := `
		...
		for _, itemID := range itemIDs {
			_, err = tx.ExecContext(ctx, queryItem, b.ID, itemID, b.ResourceID)
			if err != nil {
	...
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("repo: gagal commit transaksi: %w", err)
	}
...
func (r *Repository) GetByToken(ctx context.Context, token uuid.UUID) (*BookingDetail, error) {
	...
	// 1. Ambil data utama (Booking + Customer + Resource)
	query := `
		SELECT 
			b.*, 
	...
	// 2. Ambil detail items/options yang dipilih
...
// UpdateStatus dengan filter tenant_id (Security)
